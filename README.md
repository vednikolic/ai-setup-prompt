...
<details open>
<summary>📋 Click the copy icon (top right of the code block) to copy the full prompt</summary>
...
Explain the concept first: "Your workspace already remembers your preferences and folder structure through CLAUDE.md. If you enabled Auto Memory and Auto Dream, Claude Code writes things down and keeps those notes clean: deduplicating, fixing dates, pruning stale entries. That handles the hygiene. But it does not connect anything. Your notes are still flat files. A decision you made last month in one project and a contradiction you are about to introduce in another sit in the same memory and nobody notices. Cortex adds the intelligence layer on top of what Claude Code already does. It is a free, open source add-on that does three things: it routes what you save to the right file (decisions, learnings, daily notes) so you do not have to sort manually; it builds a knowledge graph that connects concepts across your projects so related ideas are linked, not siloed; and it surfaces cross-project signals: convergences, contradictions, and dormant ideas you keep mentioning but never act on. Auto Dream keeps your memory clean. Cortex makes it useful."
